...
/* 			if( isset($_POST['variation_id']) && isset($_POST['quantity']) ) {
				...
				$html = '<div class="nbpm-calculator"><p class="nbpm-calculator-price"><label>'. __('Total', 'woopanel') .':</label> '. wc_price($price) .' x ' . $quantity .' = '. wc_price($total) .'</p></div>';
			}else { */
	...
	public function nbt_solutions_localize($array) {
		...
		if( is_product() ) {
			...
			$array['is_scroll'] = $price_matrix_settings['wc_price-matrix_is_scroll'];
			$array['default_attributes'] = $product->get_default_attributes();
			$array['isCalculatorText'] = $price_matrix_settings['wc_price-matrix_show_calculator'];
			$array['format'] = array(
			...
			$array['pricematrix']['total_label'] = __('Total', 'woopanel');
	...
	function is_product(){
		...
		if( get_post_meta($product->get_id(), '_enable_price_matrix', true) == 'on' && get_post_meta($product->get_id(), '_pm_num', true) && $product->is_type('variable') || get_post_meta($product->get_id(), '_enable_price_matrix', true) == 'yes' && get_post_meta($product->get_id(), '_pm_num', true) && $product->is_type('variable') ){
			...
			$_pm_show = $price_matrix_settings['wc_'.NBT_Solutions_Price_Matrix::$plugin_id.'_show_on'];
			if($_pm_show != 'default'){
			...
			if($price_matrix_settings['wc_'.NBT_Solutions_Price_Matrix::$plugin_id.'_is_heading']){
			...
			if( $price_matrix_settings['wc_'.NBT_Solutions_Price_Matrix::$plugin_id.'_hide_info'] == 'yes' ){
