...
/* 				echo '<pre>';
	...
	public function save_price_find_attributes($product, $attrs) {
		...
		// Check cache post_id
/* 		if( $post_id ) {
			$check = "SELECT posts.ID as id, posts.post_parent as parent FROM {$wpdb->posts} as posts";
			$check .= " WHERE posts.ID = '". $post_id ."' AND posts.post_parent = '" . $product->get_id() . "' AND posts.post_type IN ( 'product', 'product_variation' ) AND posts.post_status = 'publish'";

			if( ! $rs = $wpdb->get_row($sql) ) {
