<div id="price_matrix_table" data-count="<?php echo $total_attributes;?>" data-product_variations="<?php echo htmlspecialchars( wp_json_encode( $new_attribute ) );?>">
	<form action="" method="POST" id="frm-price-matrix">
		...
		<div id="message" class="max-attribute-notice notice woocommerce-message">
			<p><?php _e( 'Currently, Price Matrix only support display maximum is 4 attributes to price matrix table', 'woopanel' ); ?></p>
			<p><?php _e('By default, attribute at the bottom will not be displayed to the table, it will just show in selection mode. You can choose which attribute to be shown in the table by click Show/hide next to it', 'woopanel');?></p>
		</div>
		<?php }?>
